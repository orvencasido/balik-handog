...
export default function AnalyticsPage() {
  ...
  return (
    <div className="flex-1 space-y-4 sm:space-y-6 font-sans w-full">
      {/* Dynamic Header */}
      <header className="flex flex-col gap-3 sm:gap-4 bg-white px-4 sm:px-6 py-4 sm:py-6 rounded-2xl border border-gray-100 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
          <div>
            <h1 className="text-base sm:text-lg font-black text-emerald-950 tracking-tight leading-none uppercase">Advanced Analytics</h1>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <div className="bg-zinc-50 px-3 py-1.5 rounded-lg border border-gray-100 flex items-center gap-2">
              ...
              <select value={selectedDept} onChange={(e) => setSelectedDept(e.target.value)} className="bg-transparent text-[10px] font-black text-emerald-950 outline-none max-w-[100px] sm:max-w-[140px]">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        {[
          ...
          <div key={i} className="bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-gray-100 shadow-sm group hover:border-emerald-200 transition-all">
            <h3 className="text-[7px] sm:text-[8px] font-black text-emerald-900/40 uppercase tracking-widest leading-none mb-1">{s.label}</h3>
            <p className="text-base sm:text-xl font-black text-emerald-950 tabular-nums group-hover:text-emerald-600 transition-colors whitespace-nowrap">{s.val}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="bg-white p-4 sm:p-6 rounded-2xl border border-gray-100 shadow-sm">
          <h2 className="text-[9px] sm:text-[10px] font-black text-emerald-950 uppercase tracking-widest mb-4 sm:mb-6 flex items-center gap-3">
            ...
            {sortedMonths.map((m) => (
              <div key={m.key} className="group">
                ...
                <div className="flex items-center gap-3 sm:gap-4">
                  ...
                  <span className="text-[8px] sm:text-[9px] font-black text-zinc-300 w-8">{m.count}tx</span>
                </div>
              </div>
            ))}
        ...
        <div className="bg-white p-4 sm:p-6 rounded-2xl border border-gray-100 shadow-sm">
          ...
          <div className="divide-y divide-gray-50">
            {topContributors.map((c, i) => (
              <div key={c.name} className="py-3 sm:py-5 flex items-center justify-between group">
                <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                  <span className="text-xs font-black text-emerald-900/20 tabular-nums shrink-0">0{i + 1}</span>
                  <div className="min-w-0">
                    <p className="text-[10px] sm:text-xs font-black text-emerald-950 uppercase tracking-tight group-hover:text-emerald-600 transition-colors truncate">{c.name}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="bg-white p-4 sm:p-6 rounded-2xl border border-gray-100 shadow-sm">
          <h2 className="text-[9px] sm:text-[10px] font-black text-emerald-950 uppercase tracking-widest mb-4 sm:mb-6">Weekly Heatmap</h2>
        ...
        {/* Sector Ranking */}
        <div className="lg:col-span-2 bg-emerald-900 p-4 sm:p-6 rounded-2xl shadow-xl shadow-emerald-900/20 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 sm:p-6 opacity-10">
            <svg className="w-20 sm:w-32 h-20 sm:h-32 text-white" fill="currentColor" viewBox="0 0 24 24">
          ...
          <h2 className="text-[9px] sm:text-[10px] font-black text-white uppercase tracking-widest mb-4 sm:mb-6 flex items-center gap-3">
